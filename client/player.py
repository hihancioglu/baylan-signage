...
"""
    ...
    def _stop_detached_widget_browser_processes(self) -> None:
        ...
        script = (
            ...
            "$_.Name -in @('msedge.exe','chrome.exe','msedgewebview2.exe') -and $_.CommandLine"
            "};"
            "foreach($p in $procs){"
            "$cmd=$p.CommandLine;"
            "if($Mode -eq 'engine'){"
            "if((($cmd -match '--kiosk') -or ($p.Name -eq 'msedgewebview2.exe')) -and "
            "($cmd -match 'widget_engine\\.html') -and ($cmd -match 'config_b64=')){"
            "Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue"
            "}"
            "} else {"
            "if((($cmd -match '--kiosk') -or ($p.Name -eq 'msedgewebview2.exe')) -and ($cmd -match $markerSafe)){"
            "Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue"
            "}"
            "}"
            "}"
