...
class TestWidgetViewer(unittest.TestCase):
    ...
    def test_widget_engine_fits_production_canvas_without_dpr_or_reload(self):
        ...
        self.assertNotIn("scale(${scaleX}", fit_helper)
        self.assertNotIn("scale(${scaleY}", fit_helper)
        self.assertNotIn("contentZoom", fit_helper)
        self.assertNotIn("finalScale", fit_helper)
        self.assertNotIn("Math.min(1", fit_helper)
        self.assertNotIn("object-fit: cover", fit_helper)
    ...
    def test_widget_engine_production_stage_is_centered_and_contained(self):
        engine = Path("client/widget_engine.html").read_text(encoding="utf-8")
        fit_helper = engine.split("function setupProductionAutoFit", 1)[1].split("function debugLog", 1)[0]
    ...
    def test_widget_engine_runtime_optimizes_production_grid_without_navigation(self):
        ...
        self.assertIn("score > selected.score * 1.005", optimizer)
        self.assertIn("score >= selected.score * 0.995", optimizer)
        ...
        self.assertNotIn("location.reload", optimizer)
        self.assertNotIn("location.replace", optimizer)
        self.assertNotIn("fit_content_zoom", optimizer)
